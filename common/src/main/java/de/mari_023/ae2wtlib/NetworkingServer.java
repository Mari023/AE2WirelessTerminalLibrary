...
public class NetworkingServer {
    public static void registerServer() {
        ServerPlayNetworking.registerGlobalReceiver(new ResourceLocation(AE2wtlib.MOD_NAME, "cycle_terminal"),
                (server, player, handler, buf, sender) -> {
                    buf.retain();
                    server.execute(() -> {
                        final AbstractContainerMenu screenHandler = player.containerMenu;

                        if (!(screenHandler instanceof AEBaseMenu))
                            return;
        ...
        ServerPlayNetworking.registerGlobalReceiver(new ResourceLocation(AE2wtlib.MOD_NAME, "hotkey"),
                (server, player, handler, buf, sender) -> {
                    buf.retain();
                    server.execute(() -> {
                        ...
                        if (terminalName.equalsIgnoreCase("toggleRestock")) {
                            ...
                            if (locator != null)
                                WUTHandler.updateClientTerminal(player, locator, terminal.getTag());
